...
2. **区检查**：核对绿黄红区（`docs/workflow/forking-contract.md` §2）——绿区直接做；黄区先读对应文档；红区（`BasePredictor` 签名 / 分层方向；装配了 envelope 机制的工程还包括 envelope 格式与状态码语义）停下重新评估
...
- 装配了 envelope 机制的工程：HTTP 永远 200 + `{code, message, data}` envelope；Pydantic 校验失败折叠为 code 1（422 永不泄漏）
- 状态码双注册（`tests/test_response.py` 会替你盯着）
