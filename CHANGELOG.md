...
- **README Quick Start repurposed for agent-driven development**: the three-step flow is now git clone the template → Claude Code in the template dir initializes your project at a target path → Claude Code in the new project dir keeps developing (the worked dialogue kept, with concrete `claude` invocations); running the template's demo service moves to a new Run the Demo section and the Developing with Agents section is absorbed into Quick Start; a note clarifies that new projects are full copies (canonical references live locally; the template dir only serves initialization and updates)
