...
INFERFORGE_PIPELINE=1 ./start.sh                   # + sync pipeline api (detect→crop→classify; needs yolov8n.onnx + yolov8n-cls.onnx)
python3 scripts/test_sync_pipeline.py --image assets/bus.jpg     # test the sync pipeline API
python3 scripts/run_pipeline.py --image assets/bus.jpg           # task layer directly, no web (pipeline)
INFERFORGE_DEDUP=1 ./start.sh                     # + sync dedup api (needs models/dino2-small.onnx)
python3 scripts/test_sync_dedup.py --image assets/bus.jpg --image assets/bus.jpg  # test the sync dedup API
python3 scripts/run_dedup.py --image assets/bus.jpg --image assets/zidane.jpg     # task layer directly, no web (dedup)
python3 scripts/build_gallery.py                  # build the gallery index (worker must be stopped; gallery/ -> data/gallery.db)
python3 scripts/run_search.py --image assets/bus.jpg --check      # task layer directly (gallery search / dupcheck)
...
- Sync segment/classify/pipeline are opt-in capabilities (`INFERFORGE_SEG=1` / `INFERFORGE_CLS=1` / `INFERFORGE_PIPELINE=1`, off by default, independent of the async stack; detection is always on). Switches gate which HTTP routes exist — they do NOT pick models (that's the registry's job, see the registry bullet). Segment returns the overlay JPEG + one full-image binary-mask PNG per instance (large JSON on dense scenes — documented caveat); classify returns top-5 text, no image. Pipeline (`/predict/pipeline`) is a task-layer composition: it owns NO predictors — it reuses `tasks.detection` + `tasks.classification` caches (like tasks.agent reuses detection); the request has NO `model` field (always the two capabilities' registry defaults); `INFERFORGE_PIPELINE_TARGETS` (default `car,truck,bus`) picks the detect classes to crop + classify (not in the detect model's class table → code 3 naming the var); the classifier loads lazily on the first kept crop; readiness/preflight/warmup treat classify as enabled when EITHER `INFERFORGE_CLS` or `INFERFORGE_PIPELINE` is on; no new metric labels (the two inner engines observe their own phases).
- Embed capability (`engines/dinov2.py`, registry `embed` + `INFERFORGE_EMBED_MODEL_PATH` fallback): DINOv2-small 384-d (resize 224 + ImageNet normalize → CLS token → L2 normalize; preprocessing must match the ONNX export; official weights are CC-BY-NC-4.0 — commercial deployments substitute the backbone, the contract doesn't care). Three business tasks share it (docs/embedding.md): dedup (`/predict/dedup`, sync, `INFERFORGE_DEDUP=1`, 2-50 images, N×N cosine + `INFERFORGE_DUP_THRESHOLD` (default 0.95, shared with dupcheck, read at call time) + union-find — near-duplication is TRANSITIVE (A~B,B~C groups A/B/C even if A~C is below threshold); groups carry 0-based input-position ids + representative + confidence; identification only — deletion is the caller's job), search + dupcheck (`INFERFORGE_SEARCH=1`, requires `INFERFORGE_ASYNC=1`, query-only) over a milvus-lite gallery index (`data/gallery.db`; built by `scripts/build_gallery.py` from `INFERFORGE_GALLERY_DIR` default `gallery/`, run with the worker STOPPED — the db file is single-process exclusive, so ONLY the celery worker opens it; web readiness probes embed only when `INFERFORGE_DEDUP` is on — search alone would keep web perpetually 503). `pymilvus[milvus-lite]` is worker-only + lazily imported (requirements-async.txt; tests patch `tasks.search.search_gallery` — milvus never runs in tests); the gallery is bound to the embed DEFAULT model (no `model` request field; per-model collections are out of template scope). Search returns `{matches, count}` (top_k request param, default 5, 1-50); dupcheck returns `{found, match, threshold}` (top-1 vs the shared threshold; found=false is a normal business result). `predict_phase_seconds`/`predictor_loaded` carry task="embed". The segment engine decodes the two-output head `(1,116,8400)` + `(1,32,160,160)` with shape-based output discovery (`find_seg_outputs` — docstring flags dynamic-axis fallback); all pre/post self-written, no ultralytics import. The ImageNet-1k table in `engines/imagenet_classes.py` must stay 1000 entries in standard order (matches the exported yolov8n-cls training set). Metrics `predict_phase_seconds` carries a `task` label (detect/segment/classify/embed); `predictor_loaded` carries `task` + `model` (registered model name; empty string for non-registry callers). `observe_phase` / `mark_predictor_loaded` default to `task="detect"` so detection call sites are unchanged. `predict_phase_seconds` deliberately has NO model label — engines report phases and don't know their registry name (documented limitation). New engines are registry-ready (no model path in constructors; `load(path)` injects it).
...
- VLM/Agent/search/check are **query-only** async tasks (`INFERFORGE_LLM=1` / `INFERFORGE_AGENT=1` / `INFERFORGE_SEARCH=1`, all require `INFERFORGE_ASYNC=1`) — no sync or callback variants; callback delivery is the detection task's reference implementation, and LLM/Agent callers are active business systems that poll (query is the main path). Search/check additionally CANNOT be sync: the milvus-lite gallery db is single-process exclusive (worker-only, see the embed bullet). Worker builds a fixed server-side prompt (`INFERFORGE_LLM_PROMPT` / `INFERFORGE_AGENT_INSTRUCTIONS` override) and calls a remote OpenAI-compatible endpoint; config via `INFERFORGE_LLM_MODEL`/`INFERFORGE_LLM_API_KEY` (required) + `INFERFORGE_LLM_BASE_URL` (optional), read lazily by `tasks.vlm.get_llm_config` (tests reset `tasks.vlm._client`). The openai SDK is worker-only, imported lazily inside `tasks/vlm.py` function bodies — the web process and tests must import the vlm chain without openai installed. Code 9 (upstream LLM failure after SDK retries) is a business error. VLM workers are I/O-bound — scale with `./start_celery.sh -c N`; `worker_prefetch_multiplier` stays 1.
