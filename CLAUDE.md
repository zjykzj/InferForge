...
- Callback fires exactly once: detection business errors (code 1/2/3) are NOT retried — only network failures on the callback POST retry (3 attempts, exponential backoff). Keep it that way. `tasks.detection_callback.post_callback` is the reusable helper for any future async callback task — retry constants stay single-sourced there.
