"""POST /predict/agent/query + GET /predict/agent/query/<task_id> — async agent with client polling.
...
def submit_agent_query(request: Request, payload: QueryRequest):
    ...
    logger.info("agent query request: remote=%s has_image=%s has_url=%s",
    ...
    logger.info("agent query task submitted: task_id=%s", task.id)
