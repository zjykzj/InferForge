"""POST /predict/vlm/query + GET /predict/vlm/query/<task_id> — async VLM with client polling.

Submit a VLM task; the worker stores the result envelope in Redis; the client
polls until the envelope appears. Poll semantics: missing key -> code 4 (never
submitted or expired), pending marker -> code 5, final envelope returned
verbatim (code 0/1/2/3/9 + data). Registered only when INFERFORGE_ASYNC=1 and
...
def submit_vlm_query(request: Request, payload: QueryRequest):
    ...
    logger.info("vlm query request: remote=%s has_image=%s has_url=%s",
                request.client.host if request.client else "-",
                bool(image_b64), bool(image_url))

    try:
        task = vlm_query_task.delay(
            image_b64=image_b64,
            image_url=image_url,
        ...
        logger.exception("failed to submit vlm query task")
        return response.error("failed to submit task", code=3)

    logger.info("vlm query task submitted: task_id=%s", task.id)
...
def poll_vlm_query(task_id: str):
    ...
    if not isinstance(payload, dict):
        ...
        return response.error("internal server error", code=3)
    return response.JSONResponse(payload)  # worker-stored envelope verbatim (code 0/1/2/3/9 + data)
