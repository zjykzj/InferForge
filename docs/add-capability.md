...
> 在新工程中新增一个业务能力的完整流程：先按 §1 选形态，再按 §3 的能力文件集（footprint）逐层实现，每层都有 canonical 参照可模仿。模板没有同形态参照的场合，走 §6 契约推导路径。前提：已完成 [bootstrap.md](bootstrap.md) 的初始化与底座验收。最后更新：2026-09-10
...
完成后对照 §3 footprint 自查 + §7 验收。形态选择仍走 §2 决策表——推导的是实现，不是形态。
