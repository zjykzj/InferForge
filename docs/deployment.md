...
> 两种常见部署形态的完整方案：**线上灰度发布**（新旧版本短期并存、一个入口分流）与**测试/生产环境长期共存**（两套独立栈、各自入口）。两者目的不同、隔离级别不同，分开规划。完整的发布策略谱系（滚动/蓝绿/canary/feature flag）见领域知识文档 [release-strategies.md](release-strategies.md)。最后更新：2026-09-05

## 0. 环境与灰度是什么

> 概念速览——本节只保留本工程两种形态所需的最小概念，完整谱系与对比见领域知识文档 [release-strategies.md](release-strategies.md)。
...
**灰度分流（canary split）**：入口（nginx/ALB）按比例把生产流量导给新版本——10% → 观察无异常 → 30% → 100% → 下线旧版本。核心是**控制爆炸半径**：新版本出问题只影响小比例流量，回滚只需改权重、杀进程，一分钟完成。
