...
- **VLM/Agent 只做 query 形态**：不提供同步与回调变体——回调交付以检测任务为参照实现，而 LLM 类任务的调用方是主动业务系统，轮询是主路（见 [api.md](api.md) §12、§13）；检索/查重同样 query-only，但原因是 milvus-lite 索引单进程独占（只有 worker 能开 db，见 [embedding.md](embedding.md) §5）
...
| `INFERFORGE_SEARCH` | 未设置（禁用） | 检索 + 查重接口开关（需同时 `INFERFORGE_ASYNC=1`，否则告警跳过；worker-only——milvus-lite 索引单进程独占） | `app.py` |
| `INFERFORGE_DEDUP` | 未设置（禁用） | 同步批内去重接口开关（默认关，独立于异步栈；开启时 web 探测 embed 缺省模型） | `app.py` / `apis/health.py` |
| `INFERFORGE_EMBED_MODEL_PATH` | `models/dino2-small.onnx` | embed 模型文件路径（仅注册表文件不存在时生效） | `engines/registry.py` |
| `INFERFORGE_GALLERY_DIR` | `gallery/` | 检索/查重的 gallery 图片目录（建库脚本扫描） | `scripts/build_gallery.py` |
| `INFERFORGE_GALLERY_DB` | `data/gallery.db` | milvus-lite 索引文件（建库脚本写入、worker 打开；单进程独占，重建须停 worker） | `tasks/search.py` / `scripts/build_gallery.py` |
| `INFERFORGE_DUP_THRESHOLD` | `0.95` | 近重复阈值（查重 top-1 判定与去重 union-find 共用；"多像算重复"是业务参数，放 task 层） | `tasks/embedding.py` |
