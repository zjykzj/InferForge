...
- 预留段位：`1xxx` 请求类 / `2xxx` 外部依赖类 / `3xxx` 推理类 / `5xxx` 系统类——当前用短码，v3+ 需要细分时再启用段位
...
已知取舍：`code=3`（内部错误）暂以 200 返回，会丢失 5xx 监控告警能力。当监控体系（Prometheus 等）在 v4 落地时，可评估将 `code=3` 改为 HTTP 500 的折中方案（业务错误保持 200）。
