"""Smoke tests for /predict/query — no broker, no redis, no model needed."""
import base64
import json
...
def fake_delay(monkeypatch):
    ...
    monkeypatch.setattr(detection_query.detect_query_task, "delay", _delay)
    return calls
...
def test_task_stores_success_envelope(monkeypatch, fake_redis):
    ...
    assert len(stored["data"]["detections"]) == 1
...
def test_task_stores_download_failure_envelope(monkeypatch, fake_redis):
    ...
    result = detection_query.detect_query_task.run(image_url="http://x/y.jpg")
...
def test_task_redis_failure_propagates(monkeypatch, fake_redis):
    ...
    with pytest.raises(RuntimeError):
        detection_query.detect_query_task.run(image_b64=_tiny_image_b64())
